<template>
  <ApplicationTypeView title="«Отказное письмо»">
    <div class="application-content">
      <p class="text-body-1 mb-4">
        Заявки на получение отказных/негативных писем
      </p>
      
      <!-- Application Form -->
      <v-form ref="form" @submit.prevent="submitApplication">
        <v-card class="mb-4" variant="outlined" elevation="0">
          <v-card-text>
            <div class="text-subtitle-1 font-weight-medium mb-3">Информация о заявке</div>
            
            <v-select
              v-model="application.rejectionType"
              label="Тип отказного письма"
              :items="[
                'Отказное письмо по ТР ТС',
                'Отказное письмо по ГОСТ Р',
                'Отказное письмо по обязательной сертификации',
                'Отказное письмо по декларированию'
              ]"
              variant="outlined"
              density="comfortable"
              class="mb-3"
              hide-details
            ></v-select>
            
            <v-text-field
              v-model="application.productName"
              label="Наименование продукции"
              variant="outlined"
              density="comfortable"
              class="mb-3 common-field"
              hide-details
            ></v-text-field>
            
            <v-text-field
              v-model="application.manufacturer"
              label="Изготовитель"
              variant="outlined"
              density="comfortable"
              class="mb-3 common-field"
              hide-details
            ></v-text-field>
            
            <v-text-field
              v-model="application.tnved"
              label="Код ТН ВЭД"
              variant="outlined"
              density="comfortable"
              class="mb-3"
              hide-details
            ></v-text-field>
            
            <v-textarea
              v-model="application.rejectionReason"
              label="Основание для отказа"
              variant="outlined"
              density="comfortable"
              class="mb-3"
              hide-details
              rows="3"
            ></v-textarea>
          </v-card-text>
        </v-card>
        
        <div class="d-flex gap-2">
          <v-btn
            type="submit"
            color="primary"
            variant="elevated"
          >
            СОЗДАТЬ ЗАЯВКУ
          </v-btn>
          <v-btn
            color="grey"
            variant="outlined"
            @click="cancel"
          >
            ОТМЕНИТЬ
          </v-btn>
        </div>
      </v-form>
    </div>
  </ApplicationTypeView>
</template>

<script setup>
import { ref } from 'vue'
import { useRouter } from 'vue-router'
import ApplicationTypeView from '../components/ApplicationTypeView.vue'

const router = useRouter()
const form = ref(null)

const application = ref({
  rejectionType: '',
  productName: '',
  manufacturer: '',
  tnved: '',
  rejectionReason: '',
})

const submitApplication = () => {
  // Here you would submit the application to your backend
  console.log('Submitting rejection letter application:', application.value)
  
  // Navigate back to applications list
  router.push('/applications')
}

const cancel = () => {
  router.push('/applications')
}
</script>

<style scoped>
.application-content {
  max-width: 800px;
}

/* Common fields styling */
.common-field :deep(.v-field) {
  background-color: rgb(230, 242, 255) !important; /* Light blue background */
  border: 1px solid rgb(200, 225, 255) !important; /* Add a border for more visibility */
}
</style>
